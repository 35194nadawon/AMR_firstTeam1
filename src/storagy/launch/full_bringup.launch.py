...
터미널 여러 개에서 따로 실행하던 노드들을 한 번에 실행한다:
  ...
  7. guide_nav_node + guide_controller (LLM 안내 주행 — move_to_location/출발)
...
사용법:
  ...
  ros2 launch storagy full_bringup.launch.py enable_guide:=false
"""
...
def generate_launch_description():
    ...
    guide_share = get_package_share_directory('storagy_guide')
    guide_params = os.path.join(guide_share, 'params', 'guide_nav.yaml')
    ...
    enable_guide = LaunchConfiguration('enable_guide')
    ...
    # LLM move_to_location / 출발 → guide_nav_node (Nav2 goal)
    guide_integrated = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(guide_share, 'launch', 'guide_integrated.launch.py')),
        launch_arguments={
            'params_file': guide_params,
            'use_sim_time': 'true',
            'demo_person_arrived': 'false',
            'target_x': '3.84',
            'target_y': '-0.5',
            'target_yaw': '3.14159',
        }.items(),
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'enable_guide', default_value='true',
            description='true=안내팀 guide_nav_node+guide_controller 자동 기동'),
        simulation,
        ...
        TimerAction(
            ...
            actions=[
                GroupAction(
                    actions=[guide_integrated],
                    condition=IfCondition(enable_guide),
                ),
            ],
        ),
        TimerAction(
